...
const demoActionOverview = ({ data, status, title, team, showLastlogin, reqText, task, processRequest, props , unShowUnaddress}) => {
  ...
  return (
    <div className="card mh-ac m-b-15 demoChanges">        
      <div className="cursor-pointer m-b-10" onClick={() => handleRedirectApproval(task, processRequest)}>        
      <div className="leftBox">
        <h3 className="text-blue mb-0">{data.count} <small className="fs-12 colorGray">{status}</small></h3>
        <div className="action-summ db-grey">{title}{reqText ? <span className="req-text">{reqText}</span> : ""}</div>
        </div>        
        <div className="adminIcon">{!team ? <span class="icon-single-user fs-18"></span> : <span class="icon-team fs-18"></span>}</div>
      </div>
      <div className="flex cursor-pointer" onClick={() => handleRedirectLast24Hr(task, processRequest)}>
        <div className="flex-1">
          <div className="once-pt fw-600">{dayCount}</div>
...
export default demoActionOverview
